...
ALCdevice * ofxOpenALSoundPlayer::alDevice = 0;
ALCcontext * ofxOpenALSoundPlayer::alContext = 0;
//vector<float> ofxOpenALSoundPlayer::window;
//float ofxOpenALSoundPlayer::windowSum=0;


kiss_fftr_cfg ofxOpenALSoundPlayer::systemFftCfg=0;
vector<float> ofxOpenALSoundPlayer::systemWindowedSignal;
vector<float> ofxOpenALSoundPlayer::systemBins;
vector<kiss_fft_cpx> ofxOpenALSoundPlayer::systemCx_out;

static set<ofxOpenALSoundPlayer*> players;

void ofOpenALSoundUpdate_TimelineAdditions(){
	alcProcessContext(ofxOpenALSoundPlayer::alContext);
}
...
void ofxOpenALSoundPlayer::initialize(){
	if(!alDevice){
...
//---------------------------------------
void ofxOpenALSoundPlayer::createWindow(int size){
...
//---------------------------------------
void ofxOpenALSoundPlayer::close(){
	if(alDevice){
...
// ----------------------------------------------------------------------------
bool ofxOpenALSoundPlayer::sfReadFile(string path, vector<short> & buffer, vector<float> & fftAuxBuffer){
	SF_INFO sfInfo;
	SNDFILE* f = sf_open(path.c_str(),SFM_READ,&sfInfo);
	if(!f){
		ofLog(OF_LOG_ERROR,"ofxOpenALSoundPlayer: couldnt read " + path);
		return false;
	}
	...
	if (subformat == SF_FORMAT_FLOAT || subformat == SF_FORMAT_DOUBLE){
		...
		if(frames_read<sfInfo.frames){
			ofLog(OF_LOG_ERROR,"ofxOpenALSoundPlayer: couldnt read buffer for " + path);
			return false;
		}
		sf_seek(f,0,SEEK_SET);
		frames_read = sf_readf_float(f,&fftAuxBuffer[0],sfInfo.frames);
		if(frames_read<sfInfo.frames){
			ofLog(OF_LOG_ERROR,"ofxOpenALSoundPlayer: couldnt read fft buffer for " + path);
			return false;
		}
	}
	sf_close(f);
...
#ifdef OF_USING_MPG123
//------------------------------------------------------------
bool ofxOpenALSoundPlayer::mpg123ReadFile(string path,vector<short> & buffer,vector<float> & fftAuxBuffer){
	int err = MPG123_OK;
	...
	if(encoding!=MPG123_ENC_SIGNED_16){
		ofLog(OF_LOG_ERROR,"ofxOpenALSoundPlayer: unsupported encoding");
		return false;
	}
	samplerate = rate;
...
//------------------------------------------------------------
bool ofxOpenALSoundPlayer::sfStream(string path,vector<short> & buffer,vector<float> & fftAuxBuffer){
	if(!streamf){
		SF_INFO sfInfo;
...
#ifdef OF_USING_MPG123
//------------------------------------------------------------
bool ofxOpenALSoundPlayer::mpg123Stream(string path,vector<short> & buffer,vector<float> & fftAuxBuffer){
	if(!mp3streamf){
		int err = MPG123_OK;
...
//------------------------------------------------------------
void ofxOpenALSoundPlayer::stream(string fileName, vector<short> & buffer){
#ifdef OF_USING_MPG123
...
void ofxOpenALSoundPlayer::readFile(string fileName, vector<short> & buffer){
#ifdef OF_USING_MPG123
...
//------------------------------------------------------------
bool ofxOpenALSoundPlayer::loadSound(string fileName, bool is_stream){
    ...
    if(channels == 0){
        ofLogError("ofxOpenALSoundPlayer -- File not found");
        return false;
    }
	...
	if(channels==1){
		...
		if (alGetError() != AL_NO_ERROR){
			ofLog(OF_LOG_WARNING,"ofxOpenALSoundPlayer: openAL error reported generating sources for " + fileName);
			return false;
		}
		...
		for(int i=0; i<(int)buffers.size(); i++){
			...
			if (alGetError() != AL_NO_ERROR){
				ofLog(OF_LOG_ERROR,"ofxOpenALSoundPlayer: error creating buffer");
				return false;
			}
			if(isStreaming){
				stream(fileName,buffer);
		...
		if(isStreaming){
			for(int s=0; s<2;s++){
				for(int i=0;i<channels;i++){
					...
					if (alGetError() != AL_NO_ERROR){
						ofLog(OF_LOG_ERROR,"ofxOpenALSoundPlayer: error creating stereo buffers for " + fileName);
						return false;
					}
		...
		for(int i=0;i<channels;i++){
			if (alGetError() != AL_NO_ERROR){
				ofLog(OF_LOG_ERROR,"ofxOpenALSoundPlayer: error creating stereo sources for " + fileName);
				return false;
			}
    ...
    getSpectrum(defaultSpectrumBandwidth);
    setFFTLogAverages();
    averageSize = getAverages().size();
    
    
    
    
	ofLogVerbose("ofxOpenALSoundPlayer: successfully loaded " + fileName);
	
	return true;
}

//------------------------------------------------------------
void ofxOpenALSoundPlayer::threadedFunction(){
	vector<vector<short> > multibuffer;
	multibuffer.resize(channels);
...
//------------------------------------------------------------
void ofxOpenALSoundPlayer::update(ofEventArgs & args){
    if(bMultiPlay){
...
//	ofRemoveListener(ofEvents.update,this,&ofxOpenALSoundPlayer::update);
	if(isLoaded()){
        ofRemoveListener(ofEvents().update,this,&ofxOpenALSoundPlayer::update);
...
//------------------------------------------------------------
vector<short> & ofxOpenALSoundPlayer::getBuffer(){
	return buffer;
}

//------------------------------------------------------------
void ofxOpenALSoundPlayer::setVolume(float vol){
	volume = vol;
...
//------------------------------------------------------------
void ofxOpenALSoundPlayer::setPan(float p){
	if(sources.empty()) return;
	if(channels==1){
		p=p*2-1;
		float pos[3] = {p,0,0};
...
//------------------------------------------------------------
void ofxOpenALSoundPlayer::setSpeed(float spd){
	for(int i=0;i<channels;i++){
...
void ofxOpenALSoundPlayer::setMultiPlay(bool bMp){
	if(isStreaming && bMp){
		ofLog(OF_LOG_WARNING,"ofxOpenALSoundPlayer: no support for multiplay streams by now");
		return;
	}
	bMultiPlay = bMp;		// be careful with this...
	if(sources.empty()) return;
//	if(bMultiPlay){
		ofAddListener(ofEvents().update,this,&ofxOpenALSoundPlayer::update);
//	}else{
//		ofRemoveListener(ofEvents().update,this,&ofxOpenALSoundPlayer::update);
//	}
}
...
void ofxOpenALSoundPlayer::play(){
	...
	if (bMultiPlay && !bLoop){
		...
		if (alGetError() != AL_NO_ERROR){
			ofLog(OF_LOG_ERROR,"ofxOpenALSoundPlayer: error creating multiplay stereo sources");
			return;
		}
		for(int i=0;i<channels;i++){
		...
		if (alGetError() != AL_NO_ERROR){
			ofLog(OF_LOG_ERROR,"ofxOpenALSoundPlayer: error assigning multiplay buffers");
			return;
		}
	}
...
// ----------------------------------------------------------------------------
void ofxOpenALSoundPlayer::initFFT(int bands){
...
// ----------------------------------------------------------------------------
void ofxOpenALSoundPlayer::initSystemFFT(int bands){
...
float * ofxOpenALSoundPlayer::getCurrentBufferSum(int size){
...
vector<float>& ofxOpenALSoundPlayer::getCurrentBuffer(int _size)
{
...
vector<float>& ofxOpenALSoundPlayer::getBufferForFrame(int _frame, float _fps, int _size)
{
	if(int(currentBuffer.size()) != _size)
    {
        currentBuffer.resize(_size);
...
// ----------------------------------------------------------------------------
vector<float>& ofxOpenALSoundPlayer::getSpectrum(int bands){
...
// ----------------------------------------------------------------------------
vector<float>& ofxOpenALSoundPlayer::getAverages(){
...
float ofxOpenALSoundPlayer::calculateAverage(float lowFreq, float hiFreq) {
...
int ofxOpenALSoundPlayer::freqToIndex(float freq) {
...
// ----------------------------------------------------------------------------
void ofxOpenALSoundPlayer::setLogAverages(int minBandwidth, int bandsPerOctave){
    if(!isLoaded()){
        return;
    }
...
// ----------------------------------------------------------------------------
int ofxOpenALSoundPlayer::getMinBandwidth(){
    return currentMinBandwidth;
}

// ----------------------------------------------------------------------------
int ofxOpenALSoundPlayer::getBandsPerOctave(){
    return currentBandsPerOctave;
}

// ----------------------------------------------------------------------------
float * ofxOpenALSoundPlayer::getSystemSpectrum(int bands){
	initSystemFFT(bands);
	...
	set<ofxOpenALSoundPlayer*>::iterator it;
...
// ----------------------------------------------------------------------------
void ofxOpenALSoundPlayer::runWindow(vector<float> & signal){
...
void ofxOpenALSoundPlayer::setUseFFTEnvelope(bool useFFTEnveolope){
    useEnvelope = useFFTEnveolope;
}

bool ofxOpenALSoundPlayer::getUseFFTEnvelope(){
    return useEnvelope;
}

void ofxOpenALSoundPlayer::setFFTDampening(float damp){
    dampening = damp;
}

float ofxOpenALSoundPlayer::getFFTDampening(){
    return dampening;
}

void ofxOpenALSoundPlayer::setFFTLogAverages(int minBandwidth, int bandsPerOctave){
    if(bLoadedOk){
        setLogAverages(minBandwidth, bandsPerOctave);
    }
}

int ofxOpenALSoundPlayer::getLogAverageMinBandwidth(){
    return getMinBandwidth();
}

int ofxOpenALSoundPlayer::getLogAverageBandsPerOctave(){
    return getBandsPerOctave();
}

int ofxOpenALSoundPlayer::getFFTSize(){
    return averageSize;
}
...
vector<float>& ofxOpenALSoundPlayer::getFFT(){
    float fftPosition = getPosition();
    if(bLoadedOk && lastFFTPosition != fftPosition){
        
        vector<float>& fftAverages = getAverages();
...
void ofxOpenALSoundPlayer::generateEnvelope(int size){
    
    cout<<"ofxOpenALSoundPlayer::generateEnvelope"<<endl;
    envelope.clear();
